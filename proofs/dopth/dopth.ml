...
module Deps = struct
  ...
  let add (id: Id.t) (d: Id.t) (t: t): t = 
    let deps = match IdMap.find_opt id t with
      | None -> []
      | Some deps -> d :: deps
    in
  ...
  let update (x: Id.t) (x_pth: Path.t) (tbl: t) : t=
    let f (t: t) (y, y_pth: Id.t * Path.t): t =
      if Path.is_predecessor y_pth x_pth then add y x t
      else t
    in
...
let () = 
  ...
  let tbl = List.fold_left (fun acc (x,pth) -> Deps.update x pth acc) tbl !id2path in
